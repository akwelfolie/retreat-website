...
export const people: Person[] = [
  {
    ...
    socials: [
      ...
      { name: 'facebook', href: 'https://www.facebook.com/Ashleysummersphotos/' },
  ...
  },
  {
    ...
    socials: [
      { name: 'instagram', href: 'https://www.instagram.com/mindbodyintimacystudio/' },
...
/**
 * Returns people by name. Pass one or more partial name strings to filter,
 * or call with no arguments to get everyone.
 *
 * @example
 * getPeople()                              // all people
 * getPeople('Ashley Summers')              // just Ashley
 * getPeople('Ashley Summers', 'Josephine') // Ashley + Josephine
 */
export function getPeople(...names: string[]): Person[] {
  if (names.length === 0) return [...people]
  return names
    .map((name) => people.find((p) => p.name.includes(name)))
